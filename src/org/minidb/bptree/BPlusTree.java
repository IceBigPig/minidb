...
public class BPlusTree {
    ...
    public BPlusTree(BPlusConfiguration conf, String mode, String treeFilePath) throws IOException, MiniDBException {
        ...
        // if we have to start anew, do so.
            ...
            root = new TreeLeaf(-1, -1, TreeNode.TreeNodeType.TREE_ROOT_LEAF, conf.pageSize);
     ...
     */
    @SuppressWarnings("unused")
    public void insertPair(ArrayList<Object> key, long value) throws IOException, MiniDBException, IllegalStateException {
        ...
        if (root.isFull(conf)) {
            ...
            aChild = this.root;
            TreeInternalNode node_buf = new TreeInternalNode(TreeNode.TreeNodeType.TREE_ROOT_INTERNAL, generateFirstAvailablePageIndex(conf));
     ...
     */
    private void insertNonFull(TreeNode n, ArrayList<Object> key, long value) throws IOException, MiniDBException {
        ...
        if (n.isLeaf()) {
            ...
            int iadj = (n.getCurrentCapacity() > 0 && i == 0 && conf.gt(n.getFirstKey(), key)) ? i : i - 1;
            if (n.getCurrentCapacity() > 0 && conf.eq(n.getKeyAt(iadj), key)) {// duplicate value for the same key
                if (conf.unique) {
                    throw new MiniDBException(String.format(MiniDBException.DuplicateValue, Long.toString(value), conf.keyToString(key)));
                }
     ...
     */
    private void splitTreeNode(TreeInternalNode n, int index) throws IOException, MiniDBException {
        ...
        if (ynode.isInternalNode()) {
            ...
            zInternal = new TreeInternalNode(TreeNode.TreeNodeType.TREE_INTERNAL_NODE, generateFirstAvailablePageIndex(conf));
        ...
        // we have a leaf
            ...
            zLeaf = new TreeLeaf(yLeaf.getNextPagePointer(), yLeaf.getPageIndex(), TreeNode.TreeNodeType.TREE_LEAF, generateFirstAvailablePageIndex(conf));
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private void createOverflowPage(TreeNode n, int index, long value) throws IOException, MiniDBException {
        TreeOverflow novf;
        if (n.isOverflow()) {
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private LinkedList<Long> getValues(TreeLeaf l, int i) throws IOException {
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    public LinkedList<Long> rangeSearch(ArrayList<Object> minKey, ArrayList<Object> maxKey, boolean includeMin, boolean includeMax) throws IOException, MiniDBException {
        LinkedList<Long> ans = new LinkedList<>();
        conf.padKey(minKey);
        conf.padKey(maxKey);
        if (conf.eq(minKey, maxKey) && (!includeMax || !includeMin)) {// the maxKey == minKey, but the interval is open in either side
            return ans;
        }
    ...
    private SearchResult _findKey(TreeNode current, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex, ArrayList<Object> key) throws IOException, MiniDBException {
     ...
     */
    private boolean deleteOrUpdatePair(ArrayList<Object> key, long value, long newValue, boolean delete) throws IOException, MiniDBException {
        conf.padKey(key);
        ...
        if (savedValue == value) {// the value is found in this node
            if (delete) {
                ...
                if (ovfpointer == -1) {//there is only one value for the key. delete the (key, value) pair.
                    l.removeEntryAt(i, conf);
     ...
     */
    private int canRedistribute(TreeNode with) throws MiniDBException {
        if (with != null) {
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private void redistributeNodes(TreeLeaf to, TreeLeaf with, boolean left, TreeInternalNode parent, int parentKeyIndex) throws IOException, MiniDBException {
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private void redistributeNodes(TreeInternalNode to, TreeInternalNode with, boolean left, TreeInternalNode parent, int parentKeyIndex) throws IOException, MiniDBException {
        ArrayList<Object> key;
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private TreeNode mergeNodes(TreeLeaf left, TreeLeaf right, TreeLeaf other, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex, boolean isLeftOfNext, boolean useNextPointer) throws IOException, MiniDBException {
    ...
    private void fixTheTopPointer(TreeNode other, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex, boolean isLeftOfNext, boolean useNextPointer) {
        if (useNextPointer) {
            if (isLeftOfNext) {
     ...
     */
    private void joinLeaves(TreeLeaf left, TreeLeaf right, int cap) throws MiniDBException {
        for (int i = 0; i < cap; i++) {
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private void mergeNodes(TreeLeaf left, TreeLeaf right) throws IOException, MiniDBException {
        right.setBeingDeleted(true);
        // join the two leaves together.
        int cap = right.getCurrentCapacity();
        joinLeaves(left, right, cap);
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private TreeNode mergeNodes(TreeInternalNode left, TreeInternalNode right, TreeInternalNode other, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex, boolean isLeftOfNext, boolean useNextPointer) throws IOException, MiniDBException {
     ...
     * @throws IOException is thrown when an I/O operation fails
     */
    private void mergeNodes(TreeInternalNode left, TreeInternalNode right, ArrayList<Object> midKey) throws IOException, MiniDBException {
        right.setBeingDeleted(true);
     ...
     */
    private void joinInternalNodes(TreeInternalNode left, TreeInternalNode right, int cap) throws MiniDBException {
        for (int i = 0; i < cap; i++) {
            left.addLastToKeyArray(right.popKey());
            left.addPointerLast(right.popPointer());
            left.incrementCapacity(conf);
     ...
     */
    public TreeNode mergeOrRedistributeTreeNodes(TreeNode mnode, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex) throws IOException, MiniDBException {
     ...
     * @throws MiniDBException is thrown when there are inconsistencies in the blocks.
     */
    private TreeNode handleRootRedistributionOrMerging(TreeNode mnode) throws IOException, MiniDBException {
        if (mnode.isInternalNode()) {
            ...
            if (lChild.isLeaf()) {
                ...
                */
                ...
                TreeInternalNode rNode = (TreeInternalNode) mnode, lIntNode = (TreeInternalNode) lChild, rIntNode = (TreeInternalNode) rChild;
     ...
     * @throws MiniDBException is thrown when there are inconsistencies in the blocks.
     */
    private TreeNode handleLeafNodeRedistributionOrMerging(TreeNode mnode, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex) throws IOException, MiniDBException {
        ...
        // we can't redistribute, try merging with next
            ...
            mnode = mergeNodes(splitNode, nptr, pptr, parent, parentPointerIndex, parentKeyIndex, isLeftOfNext, /*useNextPointer = */ true);
        }
        // last chance, try merging with prev
            ...
            mnode = mergeNodes(pptr, splitNode, nptr, parent, parentPointerIndex, parentKeyIndex, isLeftOfNext, /*useNextPointer = */ false);
        } else {
     ...
     * @throws MiniDBException is thrown when there are inconsistencies in the blocks.
     */
    private void validateNeighbours(TreeLeaf prev, TreeLeaf split, TreeLeaf next) throws MiniDBException {
        if (prev != null) {
     ...
     * @throws MiniDBException is thrown when there are inconsistencies in the blocks.
     */
    private TreeNode handleInternalNodeRedistributionOrMerging(TreeNode mnode, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex) throws IOException, MiniDBException {
     ...
     */
    private TreeNode.TreeNodeType getPageType(short pval) throws InvalidPropertiesFormatException {
        switch (pval) {
    ...
    private boolean isInternalNode(TreeNode.TreeNodeType nt) {
        return (nt == TreeNode.TreeNodeType.TREE_INTERNAL_NODE || nt == TreeNode.TreeNodeType.TREE_ROOT_INTERNAL);
    }
    ...
    public boolean isLeaf(TreeNode.TreeNodeType nt) {
        return (nt == TreeNode.TreeNodeType.TREE_LEAF || nt == TreeNode.TreeNodeType.TREE_LEAF_OVERFLOW || nt == TreeNode.TreeNodeType.TREE_ROOT_LEAF);
    }
     ...
     * @throws MiniDBException is thrown when there are inconsistencies in the blocks.
     */
    private void readFileHeader(RandomAccessFile r) throws IOException, MiniDBException {
        r.seek(0L);
     ...
     */
    private void deletePage(long pageIndex, boolean sort) throws IOException, MiniDBException {
